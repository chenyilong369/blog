...
但是用`key`指明了节点前后对应关系后，`React`知道`key === "ka"`的`p`在更新后还存在，所以`DOM节点`可以复用，只是需要交换下顺序。
...
对于单个节点，它会进入`reconcileSingleElement`。
...
基于以上原因，Diff 算法要经历两次遍历。

- 第一轮 Diff 算法主要用来处理更新操作。
- 第二轮 Diff 算法主要用来处理非更新操作。
